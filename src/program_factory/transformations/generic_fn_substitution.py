"""
Replaces generic functions with hard functions
"""
...
from ..type_inference import is_generic_fn, get_type
from utils import get_hard_fn_name


def substitute_generic_fns(fn_def: FunctionDefinition, fn: Function, p: Program) -> FunctionDefinition:
...
def substitute_expression(program: Program, parameter_types: dict, expression: list) -> list:
    for i, e in enumerate(expression):
        if isinstance(e, Identifier):
            if isinstance(e, Identifier) and is_generic_fn(e.name):
                fn_type = get_type(
                    e, program, parameter_types, expression[i+1:]
                )
                hf = HardFunction(
                    Identifier(get_hard_fn_name(e.name, fn_type), e.metadata),
                    fn_type,
                    e.name
                )
                expression[i] = hf.name
                if not program.has_function(hf.name):
                    program.hard_functions.append(hf)
        elif isinstance(e, list):
            expression[i] = substitute_expression(program, parameter_types, e)
        elif isinstance(e, ListLiteral):
            e.list = substitute_expression(program, parameter_types, e.list)
